...
C_num2 = correlation_numerical_dynamics_physical_3(J,W_i,W_f,t_corr_list,beta,n_as_exp,n_s_cut,n_s_noise,integration_limit)
w_list = C_an.w_list
...
axs[0,0].plot(t_list,np.real(C_num2.C_s))
axs[1,0].plot(t_list,np.imag(C_num2.C_s))
axs[0,1].plot(t_list,np.real(C_num2.C_as_fit))
axs[1,1].plot(t_list,np.imag(C_num2.C_as_fit))
...
axs2[0].plot(np.real(C_num2.C_as_minus_extra))
axs2[0].plot(np.imag(C_num2.C_as_minus_extra))
axs2[1].plot(np.real(C_num2.C_s_plus_extra))
axs2[1].plot(np.imag(C_num2.C_s_plus_extra))
...
fig5, axs5 = plt.subplots(1, 1,figsize=(14,7))
axs5.plot(np.real(C_an.fs),'b')
axs5.plot(np.real(C_num2.C_s_extra),'r--')
axs5.plot(np.imag(C_an.fs),'k')
axs5.plot(np.imag(C_num2.C_s_extra),'g--')
print("----------------------------")
print(C_num2.PM_parameters)
print("----------------------------")

fig6, axs6 = plt.subplots(1, 1,figsize=(14,7))
axs6.plot(np.real(C_an.C_s),'b')
axs6.plot(np.real(C_num2.C_s),'r--')
axs6.plot(np.imag(C_an.C_s),'k')
axs6.plot(np.imag(C_num2.C_s),'g--')

fig7, axs7 = plt.subplots(1, 1,figsize=(14,7))
axs7.plot(np.real(C_an.C_as),'b')
axs7.plot(np.real(C_num2.C_as),'r--')
axs7.plot(np.imag(C_an.C_as),'k')
axs7.plot(np.imag(C_num2.C_as),'g--')
