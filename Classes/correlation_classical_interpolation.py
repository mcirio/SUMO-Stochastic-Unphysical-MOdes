...
class correlation_classical_interpolation():
    def __init__(self,C_list,n_cut,n_noise,t_corr_list):
        ...
        self.coeff_list = self.compute_coefficients_basis(self.C_interp,self.n_cut,self.T)
    ...
    def inner_product(self,f,g,T):
        def to_integrate(x):
            return f(x) * g(x)
        return integral(to_integrate,x_i=-T,x_f=T,limit=1000) / (2*T)
    def basis(self,t,T,n):
        return np.cos(n*np.pi*t/T)
    def compute_coefficients_basis(self,C_func,n_cut,T):
        ...
        for n in progressbar(np.arange(0,n_cut+1)):
            coeff = self.inner_product(partial(self.basis,T=T,n=n), C_func, T)
            res.append(coeff)
        return res
    def generate_A(self):
        A = [] 
