...
class correlation_classical():
    def __init__(self,C_list,n_cut,n_noise,t_corr_list):
        ...
        self.coeff_list = self.compute_coefficients_basis(self.C_list,self.n_cut,self.T)
        ...
        self.corr_stochastic = self.compute_correlations_from_xi_list(self.t_corr_list,self.xi_list)
    ...
    def inner_product(self,f,g,T):
        dx = self.t_corr_list[1] - self.t_corr_list[0]
        average = 0
        for n in range(len(self.t_corr_list[:-1])):
            t0 = self.t_corr_list[n]
            t1 = self.t_corr_list[n+1]
            average += (np.real(g[n] * f(t0)) + np.real(g[n+1] * f(t1))) / 2. + 1j * (np.imag(g[n] * f(t0)) + np.imag(g[n+1] * f(t1))) / 2.
        return average * dx / (2 * T)
    ...
    def compute_coefficients_basis(self,C_list,n_cut,T):
        ...
        for n in progressbar(np.arange(0,n_cut+1)):
            coeff = self.inner_product(partial(self.basis,T=T,n=n), C_list, T)
            res.append(coeff)
        return res
    def generate_A(self):
        A = [] 
