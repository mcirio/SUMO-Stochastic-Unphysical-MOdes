...
n_as_exp = 1
n_s_cut = 500
N_corr = 5*round(n_s_cut * np.pi) #dt=2T/N_corr should be comparable (smaller) with 1/max freq=T/(pi n_s_cut). If not, the numerical integral to compute the spectral decomposition might get wrong.
n_s_noise = 1000
W_i = 0
W_f = 10 * w0
...
beta = 'inf'
# Lists
t_list = np.linspace(0,T/2.,N_corr)
...
s = sigmax
psi0_S = basis(2,0) * basis(2,0).dag()
obs_list = [sigmaz]
...
##################################################################
pickle_out = open("./PM_3/Tests/Data/Test_PM_model_BrownianZeroT_strong.dat",'wb')
dict = {}
dict = PM.save(dict)
